...
function getMilestoneDays(revenues, milestones) {
    let out = new Array(milestones.length);
    let sortedMilstones = milestones.map((e, i) => {return {milestone: e, index: i}}).sort((a,b) => a.milestone - b.milestone); // o (n log n);
    let sum = 0;
    let milestoneCtr = 0;
    for (let i = 0; i < revenues.length && milestoneCtr < milestones.length; i++){
      sum += revenues[i];
      
      while (milestoneCtr < milestones.length && sum >= sortedMilstones[milestoneCtr].milestone){
        out[sortedMilstones[milestoneCtr].index] = i + 1;
        milestoneCtr++;
      }
    }
    return out;
