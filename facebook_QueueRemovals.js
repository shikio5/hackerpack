...
function findPositions(arr, x) {
    ...
    let arrObj = arr.map((value, index) => {return {"index": (index + 1), "value": value}}); // o (n) space , o (n) time
    let result = [];
    
    while (arrObj.length > 0 && result.length < x) { // o (n) space , o (n^2) time
      let arrTemp = [];
      let largest;
      ...
      // finds largest
      while (xCtr < x && arrObj.length > 0) {
        let currentObj = arrObj.shift();
  
        if (largest == null || largest.value < currentObj.value) {
          largest = {...currentObj};
          indexOfLargest = arrTemp.length;
        }
        ...
        arrTemp.push({"index":currentObj.index, "value":currentObj.value});
              
        xCtr++;
      }
      
      if (indexOfLargest >= 0) {
         arrTemp.splice(indexOfLargest, 1);
      }
     
      result.push(largest.index);
      arrObj = arrObj.concat(arrTemp);
    }    
    
    return result;
  }
  
  // note from Danny Wu....this is disgusting.....
